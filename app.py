...
#Creating the Recall.AI Zoom bot that will be transcribing the call, listening for voice triggers, and interacting with the call
def createBot(meetingLink:str, name:str="SET DEFAULT BOT NAME"):
...
#This transcript will then be used to check for the voice command, and assemble the command prompt
...
def checkTranscript(data:[],id):
  ...
  if data:
    ...
    #iterating through each speakers transcript starting with the most recent words, scanning for the set trigger keyword 
    for speaker in splitData:
      ...
      for i in range(dataLen-1,0,-1):
        ...
        #Once the trigger word is found, pull all the words spoken in the same sentence after the trigger word to assemble the command prompt
...
#A function to send a message into the chat and interact with the call itself, other interaction methods can be found on the Recall documentation at https://recallai.readme.io/reference/recall-overview
...
def runBot(id):
  while(True):
    ...
    ADD THE ACTIONS PERFORMED AFTER THE KEYWORD TRIGGER IS FOUND AND COMMAND PROMPT ASSEMBLED HERE
    """
